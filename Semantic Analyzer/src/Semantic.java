...
public class Semantic{
    ...
    private void ProcessGraph(List <AST.class_> classes) {
        ...
        /* Steps:
         * First create the graph and check inheritance.
        ...
        int sz = 0;		// stores the number of classes
        HashMap <String, AST.class_> idxCont = new HashMap <String, AST.class_> ();             // class name → AST node
        HashMap <String, Integer> classIdx = new HashMap <String, Integer> ();                  // class name → index
        HashMap <Integer, String> idxName = new HashMap <Integer, String>();                    // index → class name
        ArrayList < ArrayList <Integer> > classGraph = new ArrayList < ArrayList <Integer> >(); // adjacency list
        ...
        classGraph.add(new ArrayList <Integer> (List.of(1)));     // Object → IO
        classGraph.add(new ArrayList <Integer>());	                  // IO has no children yet
    ...
    private void ProcessNode(AST.method method) {
        ...
        scopeTable.insert(a_self.name, a_self); // reserve self
        ...
        for(AST.formal e : method.formals) {
            ...
            if(f != null && f.getClass() == AST.attr.class) {
                ...
                reportError(filename, a_f.lineNo, "Formal parameter " + a_f.name + " has multiple definitions.");
            }
...
//        AST.attr a_self = scopeTable.lookUpLocal("self");
    ...
    private void ProcessNode(AST.let let) {
        ...
        for (AST.binding bind : let.bindings) {
            // First process the initializer expression, if any
            if (!(bind.value instanceof AST.no_expr)) {
                ProcessNode(bind.value);

                // Check that the initializer conforms to the declared type
                if (!classTable.conforms(bind.value.type, bind.type)) {
                    reportError(filename, bind.lineNo, "Inferred type " + bind.value.type +
                            " of initialization of " + bind.name + " does not conform to declared type " + bind.type);
                }
            }

            // Insert the binding into the current scope
            scopeTable.insert(bind.name, new AST.attr(bind.name, bind.type, bind.value, bind.lineNo));
        }
