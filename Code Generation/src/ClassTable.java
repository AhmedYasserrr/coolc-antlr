...
public class ClassTable {
    ...
    public ClassTable() {
        ...
        os_formals.add(new AST.formal("x", "String", 0));
        List <AST.formal> oi_formals = new ArrayList<AST.formal>();
        oi_formals.add(new AST.formal("x", "Int", 0));
    ...
    void insert(AST.class_ c) {
        ...
        for(AST.feature e : c.features) {
            if(e.getClass() == AST.attr.class) {
                ...
                if(tc_alist.containsKey(ae.name))
                    errors.add(new Error(c.filename, ae.lineNo, "Attribute " + ae.name + " has multiple definitions in class."));
                else
                ...
                if(tc_mlist.containsKey(me.name))
                    errors.add(new Error(c.filename, me.lineNo, "Method " + me.name + " has multiple definitions defined."));
                else
        ...
        for(Entry<String, AST.attr> entry : tc_alist.entrySet()) {
            if(tc.alist.containsKey(entry.getKey()))
                errors.add(new Error(c.filename, entry.getValue().lineNo, "Attribute " + entry.getValue().name + " is already defined in a parent class"));
            else {
